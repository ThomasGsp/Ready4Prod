...
class Apache:
    ...
    def conf_general(self, hostname, apachelisten):
        ...
        self.transverse.sedvalue("{SERVERNAME}", hostname, "/etc/apache2/apache2.conf")
        ...
        Apache.conf_management(["security.conf", "badbot.conf"])
    ...
    def conf_vhost(self, FILEDIR, apachelisten, VHOST):
        try:
            ...
            if VHOST["FILES"]:
                filename, file_extension = os.path.splitext(self.conf_root + VHOST["FILES"])
            ...
            Apache.site_management(["010.{servername}.conf".format(servername=VHOST["SERVER_NAME"])])
